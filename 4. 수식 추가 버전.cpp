    ...
    while (getline(src, line)) {
        ...
        if (opcode == "EQU") {
            ...
            if (operand == "*") value = LOCCTR;
                ...
                value = evaluateExpression(operand, SYMTAB); // 수식 계산
        ...
        if (opcode == "ORG") {
            if (operand.empty()) {
                ...
                if (operand == "*") {}
                    ...
                    LOCCTR = evaluateExpression(operand, SYMTAB); // 수식 계산
            }
            intermediate.push_back({ LOCCTR, label, opcode, operand });
            continue;
        }
        ...
        if (OPTAB.find(opcode) != OPTAB.end()) LOCCTR += 3;
            ...
            if (!label.empty()) {
                ...
                else value = evaluateExpression(operand, SYMTAB); // WORD 상수 수식 처리
