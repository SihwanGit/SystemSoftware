...
int main() {
    ...
    int textStart = 0;
    int textLength = 0;
    bool anyText = false; // T 레코드 존재 여부

    for (auto& l : intermediate) {
        if (l.opcode == "START" || l.opcode == "END" || l.opcode == "LTORG" || l.opcode == "EQU") continue;
    ...
    if (!anyText)
        objOut << "T " << setw(6) << setfill('0') << hex << startAddr << " 00" << endl;
